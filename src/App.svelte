...
<script>
	...
	let items = [];
	onMount(async () => {
		fetch("http://localhost:3000/blog/find/all")
	...
	async function getusername(test) {
		...
		//return 'ไม่รู้';
		//return ican;
		const response = await fetch('http://localhost:3000/user/find/id/'+test);
  		const movies = await response.json();
  		return movies.user;
	}

	async function getblockname(test) {
		const response = await fetch('http://localhost:3000/blockname/find/id/'+test);
  		const movies = await response.json();
